...
public final class PlayerChunkEnterTrigger implements Trigger, Listener {
    private final Plugin plugin;
    private final String eventId;
    private final TriggerCallback callback;
    private boolean registered;

    public PlayerChunkEnterTrigger(Plugin plugin, String eventId, TriggerCallback callback) {
    ...
    public void onMove(PlayerMoveEvent event) {
        ...
        if (from.getWorld() != to.getWorld()) {
            callback.onTrigger(new TriggerContext(eventId, to.getWorld(), to));
            return;
        }
